...
export async function diffCommand(
    ...
    console.log(chalk.dim('⏳ Requesting diff from MCP server...\n'));
    ...
    try {
        ...
        if (options.apply && response.patch) {
            console.log(chalk.yellow('\n⚠️  Apply patch functionality not yet implemented'));
...
function printResponse(response: any): void {
    ...
    console.log(chalk.cyan('║') + chalk.bold(' MCP DIFF RESPONSE ').padEnd(58) + chalk.cyan('║'));
    console.log(chalk.cyan('╚' + '═'.repeat(58) + '╝\n'));
    ...
    if (patch) {
        ...
        console.log(chalk.yellow('║') + chalk.bold(' UNIFIED DIFF PATCH ').padEnd(58) + chalk.yellow('║'));
        console.log(chalk.yellow('╚' + '═'.repeat(58) + '╝\n'));
        ...
        console.log();
        console.log(chalk.dim('💡 Tip: To apply this patch, save it to a file and run:'));
        console.log(chalk.dim('   git apply <patch-file>'));
        console.log(chalk.dim('   or'));
        console.log(chalk.dim('   patch < <patch-file>\n'));
    } else {
        console.log(chalk.yellow('⚠️  No patch generated by AI\n'));
    }
    ...
    if (model) {
        ...
        console.log(chalk.dim(`📊 Model: ${model} | Tokens: ${totalTokens} ${costStr ? `| Cost: ${costStr}` : ''}\n`));
    }
}
