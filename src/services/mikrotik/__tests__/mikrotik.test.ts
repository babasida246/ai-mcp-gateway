...
describe('MIKROTIK_COMMANDS', () => {
    it('should provide DHCP server command generator', () => {
        const cmd = MIKROTIK_COMMANDS.DHCP_SERVER_ADD('ether2', 'dhcp-pool');
    ...
    it('should provide bridge vlan command generators', () => {
        const vlanCmd = MIKROTIK_COMMANDS.BRIDGE_VLAN_ADD('br0', 100, ['ether1', 'ether2'], ['ether3']);
        expect(vlanCmd).toContain('bridge=br0');
        expect(vlanCmd).toContain('vlan-ids=100');
        expect(vlanCmd).toContain('tagged=ether1,ether2');
        expect(vlanCmd).toContain('untagged=ether3');

        const vf = MIKROTIK_COMMANDS.BRIDGE_SET_VLAN_FILTERING('br0', true);
        expect(vf).toContain('vlan-filtering=yes');
    });

    it('should provide interface state and mtu commands', () => {
        const enableCmd = MIKROTIK_COMMANDS.INTERFACE_ENABLE('ether5');
        expect(enableCmd).toContain('disabled=no');

        const mtuCmd = MIKROTIK_COMMANDS.INTERFACE_SET_MTU('ether5', 9000);
        expect(mtuCmd).toContain('mtu=9000');
    });

    it('should provide trunk/access helper commands', () => {
        const pvidCmd = MIKROTIK_COMMANDS.BRIDGE_PORT_SET_PVID('ether3', 20);
        expect(pvidCmd).toContain('pvid=20');

        const frameCmd = MIKROTIK_COMMANDS.BRIDGE_PORT_SET_FRAME_TYPES('ether3', 'admit-only-vlan-tagged');
        expect(frameCmd).toContain('frame-types=admit-only-vlan-tagged');
    });
});
